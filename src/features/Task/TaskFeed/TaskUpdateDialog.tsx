import { Autocomplete, Button, Checkbox, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, FormControlLabel, FormHelperText, Stack, TextField } from "@mui/material";
import { memo, useState } from "react";
import { CreateTaskRequest, TaskUpdateDialogProps, UpdateTaskRequest } from "~/shared/typings/tasks/tasks";
...
export const dateFormatter = (rawDate: string): string => dayjs(rawDate).format('YYYY-MM-DDTHH:mm:ss')


const TaskUpdateDialog = memo((props: TaskUpdateDialogProps) => {

    const { open, onClose, name, description, datetimeEnd, datetimeStart, satellites, isAll, id } = props;
    ...
    const initialValues: UpdateTaskRequest = {
      ...
      const validationSchema = Yup.object({
        ...
        datetimeEnd: Yup.string()
          ...
          .test('after-start', 'Дата окончания должна быть после начала', function(value) {
...
>
  <DateTimePicker
    ...
    slotProps={{
      textField: {
        error: formik.touched.datetimeStart && Boolean(formik.errors.datetimeStart),
        helperText: formik.touched.datetimeStart && formik.errors.datetimeStart,
        fullWidth: true
      },
      inputAdornment: {
        position: 'end'
  ...
  <DateTimePicker
    ...
    slotProps={{
      textField: {
        error: formik.touched.datetimeEnd && Boolean(formik.errors.datetimeEnd),
        helperText: formik.touched.datetimeEnd && formik.errors.datetimeEnd,
        fullWidth: true
      },
      inputAdornment: {
        position: 'end'
  ...
  />
                ...
                {!formik.values.isAll && (
                  <Autocomplete
                    multiple
                    options={['Спутник 1', 'Спутник 2', 'Спутник бесконечность', 'Во мрачной тьме далекого будущего нет ничего кроме спутников']} // Здесь должны быть доступные спутники
                    ...
                    renderInput={(params) => (
                      <TextField
                        ...
                        error={formik.touched.satellites && Boolean(formik.errors.satellites)}
                        helperText={formik.touched.satellites && formik.errors.satellites}
                      />
                    )}
                  />
                )}
