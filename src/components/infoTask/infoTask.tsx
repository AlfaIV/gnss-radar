...
import { signalType, task } from '~/utils/types/types'
...
const InfoTask: FC<CreateTaskProps> = ({ open, onClose, currentTask }) => {
  ...
  if (listSatellitesError || listDevicesError) {
    return (
  ...
  return (
    <Dialog fullWidth maxWidth='lg' open={open} onClose={onClose}>
      <DialogTitle variant='h4'>
        {currentTask?.id
          ? `Информация о задании №${currentTask.id}`
          : 'Информация о задании не доступна'}
      </DialogTitle>
      <DialogContent>
        <DialogContentText>Парамеры задания для устройства</DialogContentText>
        <FormControl component='fieldset' fullWidth sx={{ mt: 2 }}>
          <FormLabel component='legend'>Выберите устройство</FormLabel>
          <Typography>
            {'Название устройства: ' + task.device?.name ||
